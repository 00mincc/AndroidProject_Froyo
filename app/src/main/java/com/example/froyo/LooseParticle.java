...
public class LooseParticle extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        updateRunnable = new Runnable() {
            ...
            public void run() {
                ...
                handler.postDelayed(this, 20);  // 20ms마다 다시 실행
            }
        };
        ...
        // 10초 뒤에 LoseParticle 클래스로 이동하는 코드 추가
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                // LoseParticle 클래스로 이동
                Intent loseIntent = new Intent(LooseParticle.this, MainActivity.class);
    ...
    // 검은색 파티클을 화면의 랜덤 위치에서 생성
    private void spawnParticles() {
        // 랜덤한 생성 위치 (화면의 상단에서만 생성)
        for (int i = 0; i < numParticles; i++) {
            if (!isActive[i]) {
                // 파티클의 위치 설정 (화면 상단에서 랜덤하게)
                particleX[i] = (float) (Math.random() * bitmap.getWidth());
                particleY[i] = 0;  // 화면의 위쪽에서 시작
                speedY[i] = (float) (Math.random() * 15 + 5);  // Y 방향 속도 (떨어지는 속도)
                isActive[i] = true;
            }
        }
    }
    ...
    private void updateParticles() {
        ...
        // 각 파티클을 그리기
        paint.setColor(Color.DKGRAY);  // 파티클 색상을 어두운 회색으로 설정
        for (int i = 0; i < numParticles; i++) {
            if (isActive[i]) {
                // 파티클 그리기
                canvas.drawCircle(particleX[i], particleY[i], 8, paint);  // 크기 8dp

                // 파티클의 이동
                particleY[i] += speedY[i];  // Y 방향으로 아래로 이동

                // 화면 밖으로 나가면 사라짐
                if (particleY[i] > bitmap.getHeight()) {
